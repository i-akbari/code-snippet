...
logger.remove()
logger.remove(0) # remove only the default handler
...
1. The simplest approach is to pass keyword arguments directly to your logging methods:

you pass a keyword argument user_id=123 to logger.info() to add extra context. This extra information is then accessible in the log format using {extra}.
...
you pass a keyword argument user_id=123 to logger.info() to add extra context. 

For context that needs to persist across multiple log entries, Loguru provides two powerful methods:
- bind
- contextualize

2. bind: Creates a new logger with permanent context for all logs


3. contextualize: Temporarily adds context within a code block using a context manager
